SELECT
  ...
  CASE
    ...
    THEN (CAST(SUBSTRING_INDEX(raw.handicap, ',', -1) as decimal(2,1))
         +CAST(SUBSTRING_INDEX(raw.handicap, ',', +1) as decimal(2,1))
         ) * 0.5
    ELSE CAST(NULLIF(raw.handicap, '') as decimal(6,3))
  END                                              as handicap,
  CAST(
    CAST(NULLIF(REPLACE(REPLACE(raw.bb_bookmakers, CHAR(194), ''), CHAR(160), ''), '') as float) as integer
  )                                                as bb_bookmakers,
  raw.url
FROM
